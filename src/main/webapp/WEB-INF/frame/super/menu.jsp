      ...
      <ul id="menu" class="collapse">
          ...
          <li class='panel <c:if test="${selection == 11 || selection == 12}">active</c:if>'>
              ...
              <ul <c:choose><c:when test="${selection == 11 || selection == 12}">class="in" id="user_manage"</c:when><c:otherwise>class="collapse" id="user_manage"</c:otherwise></c:choose>>
                  <li <c:if test="${selection == 11}" >class="active"</c:if>><a href="<%=response.encodeUrl(path+"/admin/adminList?pagenation.page=1&selection=11")%>" ><i class="icon-angle-right"></i> 所有子管理员 </a></li>
          ...
          <li class="panel   <c:if test="${selection ==51 || selection ==52 || selection ==53}">active</c:if>">
             <a class="accordion-toggle collapsed"  href="#" data-parent="#menu" data-toggle="collapse"  data-target="#lessonmanage">
                  ...
                  <span class="pull-right">
	                  	<c:choose><c:when test="${selection ==51 || selection ==52 || selection ==53}"><i class="icon-angle-down"></i></c:when><c:otherwise><i class="icon-angle-left"></i></c:otherwise></c:choose>
                  </span>
              </a>
              <ul <c:choose><c:when test="${selection == 51 || selection == 52 || selection == 53}">class="in" id="lessonmanage"</c:when><c:otherwise>class="collapse" id="lessonmanage"</c:otherwise></c:choose>>
                  <li  <c:if test="${selection == 53}" >class="active"</c:if>><a href="<%=response.encodeUrl(path+"/admin/lessonList?pagenation.page=1&selection=53")%>"><i class="icon-angle-right"></i> 培训课程</a></li>
                  <li  <c:if test="${selection == 51}" >class="active"</c:if>><a href="<%=response.encodeUrl(path+"/admin/orderList?pagenation.page=1&selection=51")%>"><i class="icon-angle-right"></i> 培训订单</a></li>
                  <li  <c:if test="${selection == 52}" >class="active"</c:if>><a href="<%=response.encodeUrl(path+"/admin/dataList?selection=52")%>"><i class="icon-angle-right"></i> 数据统计</a></li>
              </ul>
          </li>
          <li class="panel   <c:if test="${selection == 61}">active</c:if>">
             <a class="accordion-toggle collapsed"  href="#" data-parent="#menu" data-toggle="collapse"  data-target="#vipmanage">
                  <i class="icon-tasks"></i> VIP管理
                  <span class="pull-right">
	                  	<c:choose><c:when test="${selection == 61}"><i class="icon-angle-down"></i></c:when><c:otherwise><i class="icon-angle-left"></i></c:otherwise></c:choose>
                  </span>
              </a>
              <ul <c:choose><c:when test="${selection == 61}">class="in" id="vipmanage"</c:when><c:otherwise>class="collapse" id="vipmanage"</c:otherwise></c:choose>>
                  <li  <c:if test="${selection == 61}" >class="active"</c:if>><a href="<%=response.encodeUrl(path+"/admin/vipList?selection=61")%>"><i class="icon-angle-right"></i> VIP收费</a></li>
              </ul>
          </li>
<!--    
